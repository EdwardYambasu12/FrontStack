...
(function($, window, document, undefined) {
    var leagues_ids =  [347, 207,152, 175,207, 300, 146, 332, 278, 28, 3, 4, 683, 377, 1, 17, 302, 168]
    ...
    $.extend(Plugin.prototype, {
        ...
        initialContent: function(liveScoreDetailsAjaxURL, action, Widgetkey, timezone) {
            ...
            $('<a href="#liveGame" class="titleWidget nav-tab">Live Matches</a>').prependTo($(liveScoreLocation));
            // Construct the All Games Tab click
            $('<a href="#allGame" class="titleWidget nav-tab nav-tab-active">Today Matches</a>').prependTo($(liveScoreLocation));
            ...
            var getTimeZoneForSlowConn = setInterval(function () {
                if (timeForFixtures.length > 0) {
                    ...
                    $('#liveScoreCalendarContainer').find('.thisDateForSelect').unbind('click').on('click', function() {
                        ...
                        $.ajax({
                            ...
                            if (!res.error) {
                                ...
                                var groubedByTeam = groupBy(sorted_array, 'team_name');
                    ...
                    //We send a request to the server for Top Matches Info
