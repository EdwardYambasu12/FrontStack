...
(function($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        ...
        initialContent: function(leagueDetailsAjaxURL, league_id, Widgetkey, league_name, leagueLogo, timezone) {
            ...
            $('<div class="league_nameCon"><div class="league_nameImg" style="background-image: url(\'' + (((leagueLogo == '') || (leagueLogo == 'null') || (leagueLogo == 'https://apiv3.apifootball.com/badges/logo_leagues/-1')) ? 'img/no-img.png' : leagueLogo) + '\');"></div><p class="league_name">' + league_name + '</p></div>').prependTo($(leagueLocation));
                ...
                if (timeForFixtures.length > 0) {
                    ...
                    $.ajax({
                        ...
                        if (!res.error) {
                            ...
                            var htmlConstructor = '<div class="tablele-container">';
                            htmlConstructor += '<p class="" style="border-left: solid 0px transparent; margin-left:auto; margin-right:auto; margin-top: 5px;">Please Check Connection</p>';
                            htmlConstructor += '</div>';
                            $('#resultsContentTable').append(htmlConstructor);
                        }
                    }).fail(function(error) {
                ...
                if (timeForFixtures.length > 0) {
                    ...
                    $.ajax({
                        ...
                        if (!res.error) {
                            ...
                            // If server dont send results we populate HTML with "Sorry, no data!"
                            var htmlConstructor = '<div class="tablele-container">';
                            htmlConstructor += '<p class="" style="border-left: solid 0px transparent; margin-left:auto; margin-right:auto; margin-top: 5px;">Opps no data!</p>';
                            htmlConstructor += '</div>';
                            $('#fixturesContentTable').append(htmlConstructor);
                        }
                    }).fail(function(error) {
